...
$null = & wsl.exe bash -lc 'echo ok'
if ($LASTEXITCODE -ne 0) {
    throw "WSL is installed but no usable Linux distro is ready. Finish WSL setup, then rerun this script."
}
...
$Command = "cd $EscapedProjectRoot && export BMADX_PROJECT_ROOT=$EscapedProjectRoot && export BMADX_TOOL_ROOT=$EscapedToolRoot && bash $EscapedToolRoot/bootstrap.sh $ArgTail"
